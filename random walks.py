...
def plotmassvol(n):
    ...
    r=[]
    N=np.logspace(1,np.log10(n),400,dtype=np.int64)
    for i in N:
    ...
    ax.set_xlabel('Mass')
    ax.set_title('Plot of mass of the total random walk against radius of to contain it')
    ...
    ax.legend()
    plt.savefig('Plot of mass of the total random walk against radius of to contain it')
    plt.show()
...
def meanslope(Samples,N):
    ...
    ax.set_ylabel('Probablity density')
    ax.legend()
    ax.set_title('Distribution of the slopes upto' +str(n)+' steps for a random walk')
    #plotting stuff:
    plt.savefig('Distribution of the slopes upto'+str(n)+' steps for a random walk')
    plt.show()
    print(str(mu) + ' with error ' +str(std))
...
def timewalk(N):
    ...
    ax.set_title('Time taken to run a random walk')
    ...
    ax.plot(Nsteps,np.multiply(np.power(Nsteps,slope-std_err),np.exp(intercept)), '.k', label='lower error')
    ax.legend()
    plt.savefig('Time taken to run a random walk')   
    plt.show()
